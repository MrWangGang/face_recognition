...
# 函数：检测人脸、对齐、裁剪
def detect_align_crop(input_path):
    # 读取图片
...
if __name__ == "__main__":
    ...
    # 处理第一张图片
    face_1 = detect_align_crop(config["image_path_1"])
    if face_1 is not None:
        vector_1 = extract_vector(face_1, feature_extractor)
    else:
        print("无法处理第一张图片，程序退出。")
        import sys
        sys.exit(1)

    # 处理第二张图片
    face_2 = detect_align_crop(config["image_path_2"])
    if face_2 is not None:
        vector_2 = extract_vector(face_2, feature_extractor)
    else:
        print("无法处理第二张图片，程序退出。")
        import sys
        sys.exit(1)
